...
socket.on("Server_Send_Give_Up_Match",function(data){
	alert("Nguoi choi"+ data + " da thoat game => ban thang")
...
function Timer_Endgame_On(){
    timer = setInterval(Endgame, 10000)
}

function Timer_Endgame_Off(){
    clearInterval(timer);
}
...
function Endgame(){
    ...
    socket.emit("Give_Up_Match", payload)
}
...
window.onunload = function () {
    ...
    //console.log("chay vo ham unload roi nef 222222222") // khong the su dung console hay alert
	
}

window.onbeforeunload = function() {
    return "Do you really want to leave PHuong "; // khong the show ra do chrome da cam 
}
